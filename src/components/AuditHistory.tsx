...
export default function AuditHistory({
  ...
  return (
    <div className="space-y-2.5">
      {history.map((audit) => {
        const isSelectedForCompare = compareIds.includes(audit.id);
        const compareIndex = compareIds.indexOf(audit.id);
        const isActive = !isComparing && activeId === audit.id;
        ...
        const handleItemClick = () => {
          if (isComparing && onToggleCompareId) {
            onToggleCompareId(audit.id);
          } else {
            onSelectAudit(audit);
          }
        };
        ...
        return (
          <div
            ...
            className={`group w-full flex items-center justify-between p-3.5 border rounded-xl cursor-pointer transition text-left ${
              isSelectedForCompare
                ? "bg-black border-emerald-400 shadow-xl shadow-emerald-950/10"
                : isActive
          ...
          >
            <div className="flex items-center gap-3 min-w-0 pr-2">
              {/* Checkbox / Compare Indicator when comparing */}
              {isComparing && (
                <div 
                  onClick={(e) => {
                    e.stopPropagation();
                    if (onToggleCompareId) onToggleCompareId(audit.id);
                  }}
                  className={`shrink-0 w-5 h-5 rounded-md border flex items-center justify-center font-mono text-[10px] font-bold transition-all ${
                    isSelectedForCompare 
                      ? "bg-emerald-400 border-emerald-400 text-black"
                      : "border-[#2A2A2D] bg-black hover:border-white text-transparent"
                  }`}
                >
                  {isSelectedForCompare ? (compareIndex === 0 ? "A" : "B") : "o"}
                </div>
              )}
            ...
            <div className="flex items-center gap-2.5 shrink-0">
              ...
              {/* Delete button (hidden or disabled during compare to avoid accidents) */}
              {!isComparing && (
                <button
