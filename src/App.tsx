...
import { analyzeCodeLocally } from "./utils/localAuditEngine";
...
import AuditComparer from "./components/AuditComparer";
...
export default function App() {
  ...
  const [codeSourceTab, setCodeSourceTab] = useState<"paste" | "upload">("paste");
  ...
  const [selectedCategory, setSelectedCategory] = useState<
    "typography" | "typographyScale" | "spacing" | "hierarchy" | "colorHarmony" | "interactions"
  >("typography");

  // Comparison states & logic
  ...
  const toggleComparisonMode = () => {
    const nextComparing = !isComparing;
    setIsComparing(nextComparing);
    if (nextComparing && compareIds.length === 0 && history.length >= 2) {
      setCompareIds([history[0].id, history[1].id]);
    }
  };

  const compareAuditA = history.find(item => item.id === compareIds[0]) || null;
  const compareAuditB = history.find(item => item.id === compareIds[1]) || null;
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 md:px-8 py-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="border border-[#2A2A2D] bg-[#141416] p-6 rounded-xl space-y-5">
            ...
            <div className="space-y-1.5">
              ...
              {fetchError && (
                <div className="text-[11px] font-mono text-amber-400 bg-amber-950/30 p-2.5 border border-amber-800/60 rounded-md space-y-1.5">
                  <p className="leading-relaxed">{fetchError}</p>
                  <button
                    type="button"
                    onClick={() => setShowCodeInput(true)}
                    className="text-[10px] text-amber-300 font-semibold underline hover:text-white cursor-pointer"
                  >
                    → Paste HTML / JSX Code directly below
                  </button>
                </div>
              )}
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <button
                  ...
                  className="text-[11px] font-mono text-[#6B6B6F] hover:text-white underline flex items-center gap-1 cursor-pointer"
                >
                  {showCodeInput ? "Close Sandbox Loader" : "Load Local Code (Files / ZIP / Folders)"}
                </button>
                {pastedCode && (
                  <div className="flex items-center gap-2">
                    <button
              ...
              <AnimatePresence>
                {showCodeInput && (
                  <motion.div
                    ...
                    className="overflow-hidden border border-[#2A2A2D] bg-[#141416]/40 p-3.5 rounded-xl space-y-3"
                  >
                    {/* Tab triggers */}
                    <div className="flex border-b border-[#2A2A2D] pb-2.5 gap-2">
                      <button
                        type="button"
                        onClick={() => setCodeSourceTab("paste")}
                        className={`px-3 py-1.5 text-[10px] font-mono font-bold rounded-lg transition-all duration-200 uppercase tracking-wider cursor-pointer ${
                          codeSourceTab === "paste"
                            ? "bg-white text-black"
                            : "text-[#6B6B6F] hover:text-white hover:bg-neutral-950/50"
                        }`}
                      >
                        Paste Raw Text
                      </button>
                      <button
                        type="button"
                        onClick={() => setCodeSourceTab("upload")}
                        className={`px-3 py-1.5 text-[10px] font-mono font-bold rounded-lg transition-all duration-200 uppercase tracking-wider cursor-pointer flex items-center gap-1.5 ${
                          codeSourceTab === "upload"
                            ? "bg-white text-black"
                            : "text-[#6B6B6F] hover:text-white hover:bg-neutral-950/50"
                        }`}
                      >
                    ...
                    {codeSourceTab === "paste" ? (
                      <textarea
                        ...
                        className="w-full h-40 bg-black text-[#E0E0E0] p-3 rounded-lg font-mono text-xs focus:outline-hidden border border-[#2A2A2D] resize-none"
                      />
                    ) : (
                      <CodeUploader
                        onCodeLoaded={(combined) => {
                          setPastedCode(combined);
                          setAuditError(null);
                        }}
                      />
                    )}
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              {history.length >= 2 && (
                <button
                  ...
                  title="Compare two historical audits"
                  className={`px-2 py-1 text-[10px] font-mono font-bold rounded-lg transition-all duration-200 uppercase tracking-wider flex items-center gap-1.5 cursor-pointer ${
                    isComparing
                      ? "bg-emerald-400 text-black hover:bg-emerald-300"
                      : "bg-neutral-900 text-[#6B6B6F] hover:text-white border border-[#2A2A2D]"
                  }`}
                >
        ...
        <div className="lg:col-span-8">
          {isLoading ? (
            ...
            <AuditComparer
              auditA={compareAuditA}
              auditB={compareAuditB}
                ...
                {activeTab === "checklist" && (
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5">
                      <MetricCard
                        title="Typography"
                        score={activeAudit.categories.typography?.score ?? 0}
                        icon={Baseline}
                        issuesCount={activeAudit.categories.typography?.issues?.length ?? 0}
                        recommendationsCount={activeAudit.categories.typography?.recommendations?.length ?? 0}
                      ...
                      <MetricCard
                        title="Type Scale"
                        score={activeAudit.categories.typographyScale?.score ?? 0}
                        icon={Scaling}
                        issuesCount={activeAudit.categories.typographyScale?.issues?.length ?? 0}
                        recommendationsCount={activeAudit.categories.typographyScale?.recommendations?.length ?? 0}
                        onClick={() => setSelectedCategory("typographyScale")}
                        isActive={selectedCategory === "typographyScale"}
                      />

                      <MetricCard
                        title="Spacing"
                        score={activeAudit.categories.spacing?.score ?? 0}
                        icon={Ruler}
                        issuesCount={activeAudit.categories.spacing?.issues?.length ?? 0}
                        recommendationsCount={activeAudit.categories.spacing?.recommendations?.length ?? 0}
                      ...
                      <MetricCard
                        title="Hierarchy"
                        score={activeAudit.categories.hierarchy?.score ?? 0}
                        icon={LayoutGrid}
                        issuesCount={activeAudit.categories.hierarchy?.issues?.length ?? 0}
                        recommendationsCount={activeAudit.categories.hierarchy?.recommendations?.length ?? 0}
                      ...
                      <MetricCard
                        title="Color Balance"
                        score={activeAudit.categories.colorHarmony?.score ?? 0}
                        icon={Palette}
                        issuesCount={activeAudit.categories.colorHarmony?.issues?.length ?? 0}
                        recommendationsCount={activeAudit.categories.colorHarmony?.recommendations?.length ?? 0}
                      ...
                      <MetricCard
                        title="Interactions"
                        score={activeAudit.categories.interactions?.score ?? 0}
                        icon={Activity}
                        issuesCount={activeAudit.categories.interactions?.issues?.length ?? 0}
                        recommendationsCount={activeAudit.categories.interactions?.recommendations?.length ?? 0}
                    ...
                    {activeAudit.categories[selectedCategory] ? (
                      <div className="border border-[#2A2A2D] bg-[#141416]/50 p-5 rounded-xl space-y-4">
                        <div className="flex items-center justify-between">
                          <h3 className="font-display font-semibold text-white text-sm md:text-base capitalize">
                            {selectedCategory === "typographyScale"
                              ? "Typography Scale (Swiss Modular Scale)"
                              : selectedCategory === "colorHarmony"
                              ? "Color Harmony"
                              : selectedCategory} Quality Analysis
                          </h3>
                      ...
                      <div className="p-4 border border-[#2A2A2D] bg-[#141416]/50 rounded-xl text-xs text-[#6B6B6F] font-mono text-center">
                        Category data unavailable for this historical audit.
                      </div>
                    )}
                      ...
                      {activeAudit.microFeedback && activeAudit.microFeedback.length > 0 ? (
                        <div className="space-y-4">
                          {activeAudit.microFeedback
                            ...
                            .filter((item) => {
                              ...
                              if (selectedCategory === "typography") {
                                return el.includes("font") || el.includes("text") || el.includes("header") || find.includes("font") || find.includes("typography") || rat.includes("font") || rat.includes("text");
                              }
                              if (selectedCategory === "typographyScale") {
                                return el.includes("size") || el.includes("scale") || el.includes("rem") || el.includes("px") || find.includes("scale") || find.includes("size") || find.includes("modular") || find.includes("swiss") || find.includes("ratio") || rat.includes("scale") || rat.includes("size") || rat.includes("modular") || rat.includes("swiss") || rat.includes("ratio");
                              }
                          ...
                          {activeAudit.microFeedback.filter(item => {
                            ...
                            const find = item.finding.toLowerCase();
                            const rat = item.rationale.toLowerCase();
                            if (selectedCategory === "typography") return el.includes("font") || el.includes("text") || el.includes("header") || find.includes("font") || find.includes("typography") || rat.includes("font") || rat.includes("text");
                            if (selectedCategory === "typographyScale") return el.includes("size") || el.includes("scale") || el.includes("rem") || el.includes("px") || find.includes("scale") || find.includes("size") || find.includes("modular") || find.includes("swiss") || find.includes("ratio") || rat.includes("scale") || rat.includes("size") || rat.includes("modular") || rat.includes("swiss") || rat.includes("ratio");
